...
def scrape_linkedin_with_adaptive_jobbert(keyword, location, filters_dict, max_pages=None, progress_msg=None):
    """Adaptive JobBERT filtering without hardcoded patterns"""
    all_scraped_jobs = []
    seen_job_ids = set()
    ...
    while True:
        ...
        if progress_msg:
            try:
                ...
                progress_text = (
                    f"🔍 **Scraping LinkedIn Jobs** {pulse_char}\n\n"
                    f"📄 Page: **{page_number + 1}**\n"
                    f"📊 Jobs Found: **{len(all_scraped_jobs)}**\n\n"
                    f"⏳ _Searching for relevant opportunities..._"
                )
                progress_msg.edit_text(text=progress_text, parse_mode=ParseMode.MARKDOWN)
            except:
                pass
    ...
    if progress_msg:
        try:
            progress_msg.edit_text("🤖 **Applying Ultra-Strict AI Filtering**\n\n⚡ _Analyzing domain requirements..._", parse_mode=ParseMode.MARKDOWN)
        except:
            pass
    ...
    if progress_msg:
        try:
            progress_text = (
                f"🤖 **Ultra-Strict AI Filtering** \n\n"
                f"📊 Original Jobs: **{len(all_scraped_jobs)}**\n"
                f"📊 After Pre-filter: **{len(jobs_to_analyze)}**\n\n"
                f"Progress: `{'█' * 10}{'░' * 10}` 50%\n"
    ...
    if progress_msg:
        try:
            completion_text = (
                f"✅ **Ultra-Strict AI Filtering Complete!**\n\n"
                f"📊 Original Jobs: **{len(all_scraped_jobs)}**\n"
                f"🎯 AI Pre-filtered: **{len(jobs_to_analyze)}**\n"
                f"✅ Final Relevant: **{len(final_jobs)}**\n"
                f"📈 Filtered Out: **{len(all_scraped_jobs) - len(final_jobs)}** ({((len(all_scraped_jobs) - len(final_jobs)) / len(all_scraped_jobs) * 100):.1f}%)\n\n"
                f"🚀 _Preparing your highly relevant results..._"
            )
            progress_msg.edit_text(text=completion_text, parse_mode=ParseMode.MARKDOWN)
        except:
            pass
...
def run_scrape(update: Update, context: CallbackContext, progress_msg):
    ...
    sorted_jobs = scrape_linkedin_with_adaptive_jobbert(search_keyword, search_location, filters, progress_msg=progress_msg)
    ...
    results_text = (
        f"🎉 **Search Results Ready!**\n\n"
        f"🎯 **{len(sorted_jobs)}** highly relevant jobs found\n"
        f"📅 Sorted by date posted (newest first)\n\n"
        f"📋 _Loading job listings..._"
    )
    progress_msg.edit_text(text=results_text, parse_mode=ParseMode.MARKDOWN)
    time.sleep(1)
...
def alert_skip_filters(update: Update, context: CallbackContext):
    ...
    try:
        baseline_jobs = scrape_linkedin_with_adaptive_jobbert(keywords, location, filter_dict, progress_msg=None)
...
def check_all_alerts(bot: Bot):
    ...
    for alert in active_alerts:
        ...
        found_jobs = scrape_linkedin_with_adaptive_jobbert(alert['keywords'], alert['location'], filter_dict, progress_msg=None)
